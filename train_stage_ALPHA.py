...
if __name__ == '__main__':
    ...
    X, _ = gen_train[0]
    print("##########################################################")
    print(X['image'].shape, X['bin_mask'].shape, X['dp_mask'].shape)
    print(list(X.keys()))

    from models_fbp import get_FBP_models
    model, layers_to_freeze = get_FBP_models(
    ...
    with open('ALPHA_v1/summary_ALPHA.txt', 'w') as fh:
    ...
    callbacks = [
        UnFreezer(base_lr, 3),
        keras.callbacks.CSVLogger('ALPHA_v1/logs_ALPHA.csv', append=False),
        keras.callbacks.ModelCheckpoint(
            'ALPHA_v1/model_ALPHA_{epoch:03d}_{val_loss:.4f}.h5',
            verbose=1,

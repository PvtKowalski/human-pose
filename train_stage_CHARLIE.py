...
if __name__ == '__main__':
    ...
    print(X['image'].shape, X['bin_mask'].shape, X['dp_mask'].shape, X['dp_points'].shape,
          "\n io shape", X['instance_offsets'].shape,
          "\n io mask shape", X['instance_offsets_loss_region'].shape,
          "\n joints mask shape", X['joints_loss_region'].shape,
          "\n joints shape", X['joints'].shape
         )
    
    from models_instance_cascade import get_full_cascade_model
    
    model, layers_to_freeze = get_full_cascade_model(
        ...
        num_joints=17,
        base_last_feats=384,
        checkpoint_DUV="BRAVO/model_BRAVO_021_0.4437.h5",
        skip_names_DUV=(
            'pyramid_relu',
            'B_2_reduced_act',
            'P_2_reduced_act',
    ...
    with open('CHARLIE_v1/summary_CHARLIE.txt', 'w') as fh:
    ...
    callbacks = [
        UnFreezer(base_lr / 5, 3),
        keras.callbacks.CSVLogger('CHARLIE_v1/logs_CHARLIE.csv', append=False),
        keras.callbacks.ModelCheckpoint(
            'CHARLIE_v1/model_CHARLIE_{epoch:03d}_{val_loss:.4f}.h5',
            verbose=1,
